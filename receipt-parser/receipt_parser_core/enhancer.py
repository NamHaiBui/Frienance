...
BASE_PATH = os.getcwd()
INPUT_FOLDER = os.path.join(BASE_PATH, "lib", "cache", "1_source_img")
TMP_FOLDER = os.path.join(BASE_PATH, "lib", "cache", "2_temp_img")
OUTPUT_FOLDER = os.path.join(BASE_PATH, "lib", "cache", "3_results")
...
def prepare_folders():
    ...
    for folder in [INPUT_FOLDER, TMP_FOLDER, OUTPUT_FOLDER]:
...
def remove_noise(img):
    ...
    img = cv2.threshold(
        cv2.GaussianBlur(img, (5, 5), 0), 150, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
    )[1]
    img = cv2.threshold(
...
def process_receipt(config, filename, rotate=True, grayscale=True, gaussian_blur=True):
    ...
    print(
        ORANGE + "~: " + RESET + "Store parsed text at: " + ORANGE + output_path + RESET
    )
...
def main():
    ...
    print(
        ORANGE + "~: " + RESET + "Found: " + ORANGE + str(len(images)),
    ...
    for image in images:
        ...
        out_path = os.path.join(OUTPUT_FOLDER, image + ".txt")
